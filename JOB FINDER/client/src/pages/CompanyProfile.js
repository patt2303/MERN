...
const CompanyForm = ({ open, setOpen }) => {
  ...
  const onSubmit = async (data) => {
    // Implement the submit logic here, e.g., dispatch an action to update the company profile
    console.log("Form submitted:", data);
    // Reset or close the form upon success
    setOpen(false);
  };
...
const CompanyProfile = () => {
  ...
  useEffect(() => {
    setInfo(companies[parseInt(params.id) - 1] ?? null);
      ...
      <div className=''>
        ...
        <div className='w-full flex flex-col md:flex-row justify-start md:justify-between mt-4 md:mt-8 text-sm'>
          ...
          <div className='flex flex-col items-center mt-10 md:mt-0'>
            <span className='text-xl'>{info?.jobPosts?.length ?? 0}</span>
            <p className='text-blue-600'>Job Posts</p>
          </div>
        </div>
      </div>
