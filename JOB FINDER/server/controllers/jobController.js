...
export const createJob = async(req, res, next) => {
    try {
        ...
        company.getJobPosts.push(job._id);
...
export const getJobPosts = async (req, res, next) => {
    try {
        ...
        // [2, 6]
        if (exp) {
            queryObject.experience = { $get: experience[0], $lte: experience[1],};
        }
        ...
        if (sort === "A-Z") {
            queryResult = queryResult.sort("jobTittle");
        }
        if (sort === "Z-A") {
            queryResult = queryResult.sort("-jobTittle");
        }
...
export const getJobById = async (req, res, next) => {
    try {
        ...
        const searchQuery = {
            $or: [
                { jobTittle: { $regex: job?.jobTitle, $options: "i" } },
